...
class ContainerClusterAutoNodeRepairEnabledRule(GcpBaseRule):
    def get_id(self) -> str:
        return 'car_gke_ensure_auto_node_repair_enabled'

    def execute(self, env_context: GcpEnvironmentContext, parameters: Dict[ParameterType, any]) -> List[Issue]:
        issues: List[Issue] = []
        for container_cluster in env_context.container_clusters:
            for node in container_cluster.node_pools:
                if not node.management.auto_repair:
                    issues.append(
                        Issue(
                            f"The {node.get_type()} `{node.get_friendly_name()}` has automatic node repair disabled",
                            container_cluster,
                            node))
        return issues
