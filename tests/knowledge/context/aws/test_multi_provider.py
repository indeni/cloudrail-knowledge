...
class TestMultiProvider(AwsContextTest):
    ...
    def test_no_provider(self, ctx: AwsEnvironmentContext):
        ...
        vpc = next((vpc for vpc in ctx.vpcs if vpc.server_name == 'us-east-1'), None)
    ...
    def test_multi_region(self, ctx: AwsEnvironmentContext):
        vpc_east = next((vpc for vpc in ctx.vpcs if vpc.server_name == 'us-east-1'), None)
        self.assertIsNotNone(vpc_east)
        vpc_west = next((vpc for vpc in ctx.vpcs if vpc.server_name == 'us-west-1'), None)
    ...
    def test_with_a_module(self, ctx: AwsEnvironmentContext):
        vpc_east = next((vpc for vpc in ctx.vpcs if vpc.server_name == 'east'), None)
        self.assertIsNotNone(vpc_east)
        vpc_west = next((vpc for vpc in ctx.vpcs if vpc.server_name == 'west'), None)
        self.assertIsNotNone(vpc_west)
    ...
    def test_with_modules(self, ctx: AwsEnvironmentContext):
        main_vpc_east1 = next((vpc for vpc in ctx.vpcs if vpc.server_name == 'east1'), None)
        self.assertIsNotNone(main_vpc_east1)
        self.assertEqual(main_vpc_east1.region, 'us-east-1')

        vpc_module_vpc_east2 = next((vpc for vpc in ctx.vpcs if vpc.server_name == 'vpc_module-east2'), None)
        self.assertIsNotNone(vpc_module_vpc_east2)
        self.assertEqual(vpc_module_vpc_east2.region, 'us-east-2')

        nested_vpc_module_vpc_east2 = next((vpc for vpc in ctx.vpcs if vpc.server_name == 'nested_vpc_module-east2'), None)
        self.assertIsNotNone(nested_vpc_module_vpc_east2)
        self.assertEqual(nested_vpc_module_vpc_east2.region, 'us-east-2')

        nested_vpc_module_vpc_west = next((vpc for vpc in ctx.vpcs if vpc.server_name == 'nested_vpc_module-west'), None)
    ...
    def test_region_from_variable(self, ctx: AwsEnvironmentContext):
        main_vpc_west1 = next((vpc for vpc in ctx.vpcs if vpc.server_name == 'us-west-1'), None)
    ...
    def test_region_from_module_variable(self, ctx: AwsEnvironmentContext):
        vpc_east1 = next((vpc for vpc in ctx.vpcs if vpc.server_name == 'east'), None)
        self.assertIsNotNone(vpc_east1)
        self.assertEqual(vpc_east1.region, 'us-east-1')

        vpc_west1 = next((vpc for vpc in ctx.vpcs if vpc.server_name == 'west'), None)
