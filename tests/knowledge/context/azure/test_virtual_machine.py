...
class TestAzureVirtualMachine(AzureContextTest):
    ...
    def test_no_os_vm(self, ctx: AzureEnvironmentContext):
        vm = next((vm for vm in ctx.virtual_machines if vm.server_name == 'cr2460-vm'))
    ...
    def test_linux_vm(self, ctx: AzureEnvironmentContext):
        ...
        if vm.is_managed_by_iac:
            self.assertEqual(vm.disk_settings.os_disk.server_name, 'azurerm_linux_virtual_machine.main.os_disk.name')
        else:
            self.assertEqual(vm.disk_settings.os_disk.server_name, 'cr2460-vm_OsDisk_1_d04be4d0ac104399a71348d74c1da7fb')
    ...
    def test_no_os_managed_disk(self, ctx: AzureEnvironmentContext):
        vm = next((vm for vm in ctx.virtual_machines if vm.server_name == 'cr2340-vm'))
        self.assertIsNotNone(vm)
        self.assertEqual(vm.os_type, OperatingSystemType.LINUX)
        self.assertIsNotNone(vm.disk_settings)
        self.assertTrue(vm.disk_settings.os_disk.is_managed_disk)
        self.assertEqual(vm.disk_settings.os_disk.server_name, 'myosdisk1')
    ...
    def test_no_os_vm_with_data_disks(self, ctx: AzureEnvironmentContext):
        ...
        self.assertTrue(all(data_disk.server_name in ('testdatadisk', 'testdatadisk2') for data_disk in vm.disk_settings.data_disks))
