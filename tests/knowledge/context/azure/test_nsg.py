...
class TestNetworkSecurityGroupUnused(AzureContextTest):
    ...
    def test_nsg_attached_to_nic(self, ctx: AzureEnvironmentContext):
        nsg = self._get_nsg(ctx)
        nic = next((nic for nic in ctx.network_interfaces if nic.server_name == 'cr2106nsg-nic'), None)
    ...
    def test_nsg_attached_to_subnet(self, ctx: AzureEnvironmentContext):
        nsg = self._get_nsg(ctx)
        subnet = next((subnet for subnet in ctx.subnets if subnet.server_name == 'cr2106nsg-snet'), None)
    ...
    def _get_nsg(self, ctx: AzureEnvironmentContext):
        nsg = next((nsg for nsg in ctx.net_security_groups if nsg.server_name in ('cr2106nsg-nsg', 'cr2460-nsg')), None)
